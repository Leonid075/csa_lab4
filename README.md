...
- **Модель вычислений:** Аккумулятор --- единственный регистр, явно доступный програмисту, все бинарные операции выполняются между аккумулятором и памятью.
...
- **Регистры процессора:**
  ...
  - `SR` (State Register) --- Регистр ростояния (для флагов N, Z, O, C).
...
АЛУ имеет два входа: `IN1` и `IN2`, выход `ALU_OUT`, и управляющие входы. Коммутатор имеет вход `COM_IN`, выход `OUT`, вывод состояния флагов и управляющие входы. Со входом АЛУ `IN1` соединён регистр `ACC`, со входом `IN2` регистры `IP`, `DR`, `SP`, `CR` и `SR`. Если все вентили от регистров к АЛУ закрыты, то на входе установлен 0. Выход АЛУ `ALU_OUT`, подаётся на вход коммутатора `COM_IN`. Выход коммутатора `OUT` подключен к регистрам, кроме `SR`, к `SR` подключен вывод состояния флагов с коммутатора. Подать адресный регистр на вход АЛУ невозможно, в него можно только записать данные. Регистр данных подключен к шине данных, к которой так же подключены память и устройства ввода вывода. Управляющие входы АЛУ, коммутатора и вентили на регистрах и шине данных подключены к устройству микропрограммного управления, от которого получают сигнал (с помощью операциооных микрокоманд).

![](data/DP.svg)

#### Control Unit

Устройство микропрограммного управления состоит из ПЗУ для микрокоманд, счётчика микрокоманд, блока управления счётчиком и шин данных от и к datapath: регистрам, памяти, устройств ввода вывода, вентилям. Прочитанная микрокоманда либо влияет на datapath (операционная микрокоманда), либо влияет на счётчик микрокоманд в соответствии с условием микрокоманды.
...
- **Управляющая** (бит = 1) управляет МПУ, используется для выбора операцияонных микрокоманд в зависимости от опкода.

|размеры в битах            |1|1  |1  |2  |2 |1|8     |8       |16    |8      |
|---|---|---|---|---|---|---|---|---|---|---|
| операциооная микрокоманда |0|hlt|0  |mem|io|0|COM_OP|REG_DST |ALU_OP|REG_SRC|
...
| ADDR | адерес микрокоманды, на которую нужно перейти, если выполнятеся условие |
